...
While I love debloat tools like the CTT WinUtil and Belim's FlyByOOBE or CrapFixer, when you work with hundreds of computers like I do, you're not doing to manually configure these options on each pc, and for each user... that's extremely inefficient and time consuming... time we don't have. 

So let's fix that and make it soo easy and safe that it removes any hurdles.

What's different from this than all the others out there: we address the problem settings across all users of a PC, and also apply the better settings to the default profile windows uses for any new accounts that are added.
Plus ONLY change settings that DON'T affect Windows features or security. Literally the safest and most comprehensive optimization & debloat tool out there.
...
This is a problem for Custom PC builders & IT people because when a new person signs in with their account on that computer, all the bad settings and bloat come back.

This script will make every safe registry setting change to Windows on a system and per-user level that helps the pc perform better, and applies specific changes using logic instead of blindly applying settings.

It disables and permanently removes common "bloat" including things like co-pilot, widgets, and also disables wasteful scheduled tasks that hog system resources.
